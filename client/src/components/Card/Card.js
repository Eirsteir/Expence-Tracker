...
class Card extends React.Component {
  ...
  render() {
    ...
    return (
      ...
      <h1 style={{fontWeight: '300', padding: '.5em', display: 'flex', justifyContent: 'center', backgroundColor: '#1769aa', color: '#fff', marginTop: '0'}} >
        {
          timespan === 'today' ? 'Today' : ( timespan === 'thisWeek' ? 'This Week' : ( timespan === 'thisMonth' ? 'This Month' : 'Total' ))
        }
      </h1>
...

      {
        ...
        ? <Typography variant="body1" component="p" className={classes.emptyList}>
          Add a new expence
          </Typography>
        : Object.keys(tags).map((tag, i) => {
          return (<Typography variant="body1" component="p" key={i} className={classes.control}>
            {` ${tag}: ${tags[tag]}`}
          </Typography>)
        })
      }
