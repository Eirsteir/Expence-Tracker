...
class ExpencesCardList extends React.Component {
  ...
  calculateTimeAndPushToArray = expence => {
    ...
    this.state.timespan.thisWeek.push(expence); // not good

    // Push to all time total expences
    return this.state.timespan.allTime.push(expence); // not good
  };

  getTimeSpan = expences => {
    if (expences.length === 0) {
      return false;
    } else {
      Object.keys(expences).map((month, i) => {
        const currentExpence = expences[month][0];
        if (currentExpence.length !== 0) {
          return currentExpence.map((exp, i) =>
            this.calculateTimeAndPushToArray(currentExpence[i])
          );
        }
        return true;
      });
